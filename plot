...
fit f(x, y, t, u) "daily-simple" using (log($2 + epsilon)):(log($58 + epsilon)):($61):(log($15 + $26 + $27 + epsilon)):(log($1 + epsilon)):(1) via \
	split_weight, \
...
stats "daily-simple" using ((f(log($2 + epsilon), log($58 + epsilon), $61, log($15 + $26 + $27 + epsilon)))):(log($1))
set logscale xy

set logscale xy
plot "daily-simple" using (exp(f(log($2 + epsilon), log($58 + epsilon), $61, log($15 + $26 + $27 + epsilon)))):1 with points ps .3, x
