...
fit f(x1, x2, x3, x4, x5, x6) "daily-simple" using (log($2 + epsilon)):(log($58 + epsilon)):(log($15 + epsilon)):(log($26 + epsilon)):(log($62 + epsilon)):($61):(log($1 + epsilon)):(1) via \
...
stats "daily-simple" using (f(log($2 + epsilon),log($58 + epsilon),log($15 + epsilon),log($26 + epsilon),log($62 + epsilon),$61)):(log($1 + epsilon))
set logscale xy

set logscale xy
set xrange [1:500000]
set yrange [1:500000]
plot "daily-simple" using (exp(f(log($2 + epsilon),log($58 + epsilon),log($15 + epsilon),log($26 + epsilon),log($62 + epsilon),$61))):(($1 + epsilon)) with points ps .3, x1
